...
def valida_login(request):
    ...
    if len(usuario) == 0:
        ...
        return redirect('sairi')   ##('/auth/sairi/') ####VERIFCAR PAGINA CORRETA      
...
def sairi(request):
    ...
    if request.session.get('usuario'):
        ...
        return render(request,'sairi.html')#, {'status': status})
   # else:
    #    return redirect('login')    ##('auth/valida_login/?status=2')
    
    #return render('sairi.html')"""   
